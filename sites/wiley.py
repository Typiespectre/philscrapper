...
def wiley_rss(url):
    ...
    try:
        ...
        print("Wiley (rss feed) - The scraping job failed. See exception: ")
        print(e)


# 갱신된 기사들의 text 전문을 가져온다.
def wiley_text(url, n, list_len):
    ...
    try:
        ...
        print("Wiley (scrapping) - The scraping job failed. See exception: ")
        print(e)
...
def wiley_scrapping(url):
    wiley_rss(url)
    n = 1
    for key in article_list:
        text = wiley_text(key["link"], n, len(article_list))
    ...
    print("Scrapping Wiley Finished!\n")
    return article_list