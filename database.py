...
c = conn.cursor()
c.execute("CREATE TABLE IF NOT EXISTS philscrapper (name text not null, title text not null, link text not null, published text not null, unique (name, title, link, published))")


for link in scrap_list.values():
    for i in link:
        c.execute(
            "INSERT OR REPLACE INTO philscrapper(name, title, link, published) VALUES(?,?,?,?)", (i['name'], i['title'], i['link'], i['published']))
