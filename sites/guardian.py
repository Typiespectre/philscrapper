...
def guardian_rss(url):
    print("\nConnecting to Guardian rss feed...")
    request_headers = {
    ...
    try:
        ...
        for a in articles:
            ...
            time = a.find("dc:date").text
            name = "Guardian"

            dateFormatter = "%Y-%m-%dT%H:%M:%SZ"
        ...
        print("Guardian (rss feed) - The scraping job failed. See exception: ")
        print(e)


# 갱신된 기사들의 text 전문을 가져온다.
def guardian_text(url, n, list_len):
    ...
    try:
        ...
        for item in soup.find_all(
            "div",
            {
                "class": "article-body-commercial-selector css-79elbk article-body-viewer-selector"
            },
        ):
            text = text + str(item.find_all(text=True))
            return text
    except Exception as e:
        print("Guardian (scrapping) - The scraping job failed. See exception: ")
        print(e)
...
def guardian_scrapping(url):
    ...
    print("Scrapping Guardian Finished!\n")
    return article_list