...
def nature_rss(url):
    ...
    try:
        ...
        print("Nature (rss feed) - The scraping job failed. See exception: ")
        print(e)


# 갱신된 기사들의 text 전문을 가져온다.
def nature_text(url, n, list_len):
    print(f"scrapping article text... {n}/{list_len}")
    text = ""
    try:
        r = requests.get(url)
        soup = BeautifulSoup(r.text, "lxml")
        for item in soup.find_all("div", {"class": "c-article-section__content"}):
            element_list = ["p"]
            text_list = [
            ...
            text = "".join(text_list)
            return text
        for item in soup.find_all("div", {"class": "article__body cleared"}):
            text = text + str(item.find_all(text=True))
            return text
    except Exception as e:
        print("Nature (scrapping) - The scraping job failed. See exception: ")
        print(e)


# 위 함수들을 하나로 통합한 중심 함수!
# rss feed로 갱신된 기사를 스크래핑하는 과정을 모두 포함하는 함수.
def nature_scrapping(url):
    nature_rss(url)
    n = 1
    for key in article_list:
        text = nature_text(key["link"], n, len(article_list))
    ...
    print("Scrapping Nature Finished!\n")
    return article_list